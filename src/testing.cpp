...
int main() {
    ...
    std::string members[] = { "elevationOffset", "latOffset", "confidence", "radiusOfCurve", "fuelType", "hpmsType", "bumpers", "height", "weatherProbe" };
    ...
    for (int i = 0 ; i < sizeof(members)/sizeof(members[0]); i++) {
        ...
        std::cout << "Is the '" << member << "' member present? " << psuccess << std::endl;
    }
